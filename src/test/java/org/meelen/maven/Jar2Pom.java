...
/**
 * @author ：liangmin
 * @date ：Created in 2019/9/25 19:09
 * @description：
 * @modified By：
 */
public class Jar2Pom {
    ...
    public static void main(String[] args) {
        ...
        File libs = new File("D:\\创兴银行信用卡\\cip\\commonLib\\lib");
        for (File jar : libs.listFiles()) {
            ...
            if (!getPomByChecksum(jar).isTextOnly()) {
                ...
                String out = getPomStr(jar.getName());
    ...
    public static String getPomStr(String jarName) {
        jarName = jarName.replace(".jar", "");
        String nofound = "";
        String[] splits = jarName.split("-");
        if (splits.length > 1) {
            int index = jarName.lastIndexOf("-");
            String begin = jarName.substring(0, index);
            String end = jarName.substring(index + 1, jarName.length());
            //System.out.println(begin);
            //System.out.println(end);
            nofound = "<dependency><groupId>not_find</groupId><artifactId>" + begin + "</artifactId><version>" + end + "</version></dependency>";
        } else {
            nofound = "<dependency><groupId>not_find</groupId><artifactId>" + jarName + "</artifactId><version>0.0.0</version></dependency>";
        }
        return nofound;
    }
