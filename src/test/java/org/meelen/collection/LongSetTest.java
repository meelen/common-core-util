...
public class LongSetTest extends TestCase{
	...
	public byte[] toByteArray(LongSet obj) {
		try (ByteArrayOutputStream bos = new ByteArrayOutputStream(obj.size() * 8); DataOutputStream writer = new DataOutputStream(bos);) {
			for (long value : obj) {
	...
	public LongSet toObject(byte[] bytes) {
		...
		try (ByteArrayInputStream bis = new ByteArrayInputStream(bytes); DataInputStream ois = new DataInputStream(bis);) {
